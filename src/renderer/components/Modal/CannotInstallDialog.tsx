                ...
                <>
                    ...
                    <div>
                        {disallowedRunningExternalApps.map((app) => (
                            <RunningExternalAppEntry addon={addon} publisher={publisher} app={app} runningExternalApps={runningExternalApps} />
                        ))}
                    </div>
        ...
        <div key={app.key} className={`flex items-center gap-x-7 bg-navy border-t-4 ${borderColor} px-7 py-6 rounded-md my-6`}>
            ...
            <div className="w-full flex justify-between items-center">
                ...
                <span className={`flex justify-between items-center gap-x-5 ${textColor}`}>
                    {canAppBeStopped ? (
                        <Button className="h-14 px-12 py-1" onClick={handleStop} disabled={!appIsRunning}>{appIsRunning ? 'Stop' : 'Stopped'}</Button>
                    ) : (
