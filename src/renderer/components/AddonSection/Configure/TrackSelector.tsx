...
export const Tracks: React.FC = ({ children }) => (
  <div className="flex flex-row justify-start items-stretch gap-3">{children}</div>
);
...
export const Track: React.FC<TrackProps> = ({ isSelected, isInstalled, handleSelected, addon, track }) => {
  ...
  return (
    ...
    >
      <div
        className={`w-1 h-12 rounded-r-xl transition-all duration-200 transform ${isSelected ? 'scale-y-100' : 'scale-y-50'}`}
      />
      <div className="flex flex-col px-3 py-2.5">
        <span className="text-xl text-current">{track.name}</span>
        <span className="text-3xl font-manrope font-medium text-current tracking-wider mt-0.5">
          {latestVersionName}
        </span>
      </div>
      {isInstalled && <Check className={`absolute right-4 text-cyan stroke-current`} strokeWidth={3} />}
    </div>
  );
};
